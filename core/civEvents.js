...
(function (root, factory) {
  ...
  var EVENTS = [
    ...
    /* ═══════════════════════════════════════════════════════════════
       ...
       `era: 4` 的物种专属事件。通用池本身是空的。2026-09-23 报的就是这个。
       ...
       在第四纪元还是什么都没有。连玩两局都没遇到，概率 26.7%，不是运气差。
       ...
       2026-09-23 的原话：「按照逻辑加，不要因为概率问题影响属性」。
  ...
  var BIG_EVENTS = [
    ...
    /* ═══════════════════════════════════════════════════════════════
       ★★ ①`lookup`「{folk}开始仰望」—— **2026-09-22 删除**（已拍板：甲）★★
       ═══════════════════════════════════════════════════════════════
          ...
          见 `_civEvents_test.js` 里那条扫重名的断言。 */
    {
      ...
      /* ── ★★ 2026-09-22：加门槛（已拍板）—— 全项目唯一"加起来"的一个 ★★ ──

         ── 为什么判的是"三样加起来" ──
         原话是：「如果在第三纪元之前，文化、科技和工业加起来低于一个数值，
    ...
    },
    {
      ...
      /* ═══════════════════════════════════════════════════════════════
         ...
         ★★ 2026-09-22：改了后半截，**其他几个性格的加权值整个取消** ★★
         ...
         ⚠️ `生产 ≥ 1` / `科技 ≥ 2` 这两条是 2026-09-22 定的，**没有量过**：
      ...
      gate: {
        ...
        /* ⚠️ 这里**原来有 `odds`**（六种性格各一个概率），2026-09-22 
       ...
       ⚠️ 门槛在**触发那一刻**判，不在排期时判（理由同 ⑦ / ②）。 */
    {
      ...
      options: [
        ...
        /* ⚠️ 2026-09-22 已拍板：**删掉原来的第三个选项【换个位置】** ——
         ...
         ── 第二轮（2026-09-22 把整屏重抄了一遍）──
           ...
           ⚠️⚠️ 第三项**整条换掉了**：
              ...
              ⚠️ 旧版那句「那些东西只在旁边看着」是上一轮**特意保下来的好句子**，
                 这一轮被整条替换掉了 —— 要整条换，好句子也不留。
              ⚠️ 给的名字是「给工具加上限制」（**7 个字**），
   ...
   *
   *   起因是 ③`stopped`（已拍板）：「文化 + 科技 + 工业 **加起来**低于 7」。
   *
   *   ── 为什么上面那两个表达不出来 ──
   *   两个老形状判的都是「**每一样**够不够」：
   ...
   *   （2026-09-21 定的）。掷骰子会把"必定"掷没，所以让它们插队。
   *   ⚠️ 但它们**照样要过属性门槛** —— 原话是「好战这个性格**超过这个值**
  ...
  /* ═══════════════════════════════════════════════════════════════
     ...
     **现在抵抗成功再不是随机事件了**，加上这一条」
     ...
     ── ★★ 文化**不算在内**，这是有意的，不是漏写 ★★ ──
     当初只写了三项。而和文化"**不管死活、只管活成什么样**"的定位
     正好对上 —— 它是四个数里唯一不减缓任何压力线的那个。

     ── 门槛是**量的**，不是拍的（决策 #94）──
     原话里写的是 20。**实测 20 是一局都过不了的**（0.00%）——
     那正好踩中立项时立的那条红线：
  ...
  var VISIT = {
    ...
    /* ★★ 帮助给的四属性（2026-09-23 已拍板加的）★★

       **起因**：被问到「之前设定会有一部分概率是帮忙文明的吧，
  ...
  var OPTION_ATTR = {
    ...
    /* ⚠️⚠️ 规矩（2026-09-22 已拍板）：**一手就能锁定结局的选项，不加属性** ⚠️⚠️
    ...
    'depart.release': null,
    /* ★ 2026-09-22 已拍板：人都留下来了，力气还在。 */
    'depart.hold': { prod: 1 },
    'depart.mark': { cul: 1 },

    /* ★ 2026-09-22 已拍板：唤醒他们 = **科技 +1、工业 +1、文化 +1**。
   ...
   *   科技 +2 / 生产 +2（已拍板），而它**不是选项**（不挂在任何事件上），
   *   所以 `apply` 那条路走不到它。
   *
  ...
  function apply(world, eventKey, optionKey) {
    ...
    /* ═══════════════════════════════════════════════════════════════
       ...
       `lockFate` 的值是**一个 `FATES` 的 key**，现在有两个：
    ...
    if (opt.invaded) {
      /* ★★ 2026-09-15：从**掷骰**改成**确定性判定** ★★
         ...
         ⚠️⚠️ **文化不算在内**，这是定下的、有意的 —— 见 RESIST_NEED 那段。
  ...
  /* ═══════════════════════════════════════════════════════════════
     ...
     起因是的一句「军事涨的好慢，是不是事件链断了」。量完（`_attr_probe.js`
     736 局）发现：**链子没断，是偏科** ——
