...
(function (root, factory) {
  ...
  /* ═══════════════════════════════════════════════════════════════
     十六进制颜色的处理
     ═══════════════════════════════════════════════════════════════
     元素的颜色是用 '#FF8A3C' 这种写的（跟 CSS 一致，方便调），
  ...
  function drawWorld(ctx, world, cx, cy, R, time) {
    ...
    // ── 2. 世界边缘的光晕 ──
    drawHalo(ctx, cx, cy, R, hue, world);
   ...
   *
   *  ⚠️⚠️ 2026-09-15 改过一次（用户报「那圈绿和 UI 打架」）⚠️⚠️
   *
   ...
   *     光晕反映的仍然是这颗世界自己的颜色。 */
  function drawHalo(ctx, cx, cy, R, hue, world) {
    var outer = R * 1.45;
  ...
  /**
   * 和 project 一样，但**背面的点贴到边缘**而不是丢掉。
   *
   * 为什么要贴：海洋/大陆那种斑块比半个球还大，会横跨边缘。
   * 直接丢掉背面的顶点，多边形会塌掉、出现缺口；
   * 把背面的顶点按原方向拉到边缘上，轮廓就变成"沿着边缘绕过去"，
   * 连续、不跳、不留洞。
   */
  function projectClamp(lon, lat, rot, R) {
    var c = toCam(lon, lat, rot);
    if (c.x > 0) return camToScreen(c, R);
    var m = Math.sqrt(c.y * c.y + c.z * c.z) || 1;
    return { x: (c.y / m) * R, y: -(c.z / m) * R, z: 0 };
  }
