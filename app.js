  ...
  */
  ...
  function refreshInfo() {
    ...
    /* ⚠️⚠️ 必须**连 DOM 一起清**，不能只清计数 ⚠️⚠️
       ...
       踩过的坑（2026-09-12，报上来的）：这里原来只把 chronShown 归零，
  ...
  function selectElement(key) {
    ...
    /* ★ 2026-09-21：选了元素之后，这行说明**染成那个元素的颜色**。
       原话是：「点击元素，下面会有元素的功能介绍，这个字体也不太明显，
  ...
  function showEnding() {
    ...
    /* ── 存续那一段：只有走到过文明的世界才有 ──
       ...
       ⚠️ 这里本来还有「最终压力」三格，2026-09-13 决定整个删掉，
          见 index.html 那段注释。 */
    if (result.civ) {
   ...
   *    原话是：「以默认 10 为满值，但是**超过 10 了还是继续写 11、12**，
   *    多的值不参与结局设定了，只是显示而已」。
  ...
  function syncAttrs() {
    ...
    /* ── ★★ 修真彩蛋：这四个数**观测不出来**（2026-09-16，已拍板）★★
  ...
  function syncChronicle() {
    ...
    /* 不在文明阶段（换星球 / 重置 / 还没走到）—— 收起来，缓存和 DOM 一起清。
       ...
       ⚠️ 判据看的是 **DOM 里还有没有东西**，不是缓存值。
          ...
          （2026-09-12 报上来的 bug）。看 DOM 是幂等的，不受缓存状态影响。 */
    if (!on) {
  ...
  function syncButtons() {
    ...
    /* ── 开始演化 / 暂停 / 继续：一直在，标签随运行状态变 ──
       ⚠️ 设置模式那句是「**开始**演化」不是「演化」（2026-09-14 定的）。
  ...
  /* ═══════════════════════════════════════════════════════════════
     ...
     ⚠️ **存哪儿是外壳的事，core 一律不管**（这条是早就交代过的）。
   ...
   *    2026-09-14 的原话：「星球就单存的留个图片，不是留之前的数据」。
   *
  ...
  btnEndArchive.addEventListener('click', function () {
    ...
    /* ⚠️ 这里**不发提示** —— "开始下一轮"那条由 `showEnding()` 在**结局弹出的
       ...
       ⚠️⚠️ 而且这条提示**不能**指向【观察记录】：那颗按钮带 `setup-only`，
           **观察模式里根本不显示**（2026-09-14 定稿的，见 index.html
  ...
  btnReset.addEventListener('click', function () {
    ...
    /* ★ 世界名也一起清掉（2026-09-14 改的）。

       ⚠️⚠️ 这条**推翻了当初的决定 #8** ——
          原来写的是「重置是"这颗世界重跑一遍"，不是换一颗，名字跟着世界走」。
          报上来的原话是：「点击重置后上一个星球的名字会留在页面上」。
